...
export default function ProductsPage() {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(collectionJsonLd) }} />
    <section data-hero className="sticky top-0 z-0 relative isolate min-h-screen overflow-hidden bg-hubi-deep-water text-hubi-cream">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1600px] items-start px-[9%] pb-16 pt-36 md:items-center md:px-8 md:pb-0 md:pt-16 lg:px-12"><div className="max-w-xl"><h1 className="font-serif font-medium leading-[.94] md:max-w-2xl"><FoldText text="Ra khơi theo cách của bạn." splitBy="word" hinge="top" duration={0.65} stagger={0.045} perspective={700} creaseShading={0.45} fontSize="clamp(3.25rem, 8.5vw, 4.75rem)" fontWeight={500} color="var(--hubi-cream)" /></h1><ScrollReveal delay={0.32}><p className="mt-5 max-w-lg text-base leading-7 text-hubi-cream/80 sm:text-lg md:mt-6 md:text-lg lg:text-xl">Thuyền, SUP và phụ kiện cho những ngày muốn đi xa hơn trên mặt nước.</p></ScrollReveal></div></div>
    </section>
    <ProductCatalog />
  </>
}
