...
export function NavigationMenu({ items }: { items: readonly NavigationItem[] }) {
  ...
  return <>
    <div className={`${isProductDetail ? "relative" : "fixed"} inset-x-0 top-0 z-50 hidden border-b text-hubi-cream transition-colors duration-300 md:block ${surface}`}>
    ...
    <div className={`${isProductDetail ? "relative" : "fixed"} inset-x-0 top-0 z-50 flex h-16 items-center justify-between border-b px-4 text-hubi-cream transition-colors duration-300 md:hidden ${surface}`}>
