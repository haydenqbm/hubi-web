...
export function DepthCarousel({ items, cardWidth = 360, cardHeight = 500, depth = 160, spread = 52, tilt = 18, perspective = 1400, visibleCards = 3, falloff = 0.2, blur = 0, duration = 700, loop = true, showControls = true, showIndicators = true }: Props) {
  ...
  const onPointerDown = (event: React.PointerEvent) => { if (data.length > 1) { dragRef.current = { x: event.clientX, y: event.clientY, start: positionRef.current, moved: false, id: event.pointerId }; rootRef.current?.setPointerCapture(event.pointerId) } }
  const onPointerMove = (event: React.PointerEvent) => { const drag = dragRef.current; if (!drag) return; const distance = event.clientX - drag.x; const verticalDistance = event.clientY - drag.y; if (!drag.moved && Math.abs(distance) > 6 && Math.abs(distance) > Math.abs(verticalDistance)) drag.moved = true; if (drag.moved) { event.preventDefault(); positionRef.current = drag.start - distance / Math.max(cardWidth * 0.35, 40); layout(positionRef.current) } }
