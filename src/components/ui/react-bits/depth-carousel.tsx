...
export function DepthCarousel({ items, cardWidth = 360, cardHeight = 500, depth = 160, spread = 52, tilt = 18, perspective = 1400, visibleCards = 3, falloff = 0.2, blur = 0, duration = 700, loop = true, showControls = true, showIndicators = true }: Props) {
  ...
  const dragRef = useRef<{ x: number; y: number; start: number; moved: boolean; id: number } | null>(null)
  ...
  const onPointerDown = (event: React.PointerEvent) => { if (data.length > 1) dragRef.current = { x: event.clientX, y: event.clientY, start: positionRef.current, moved: false, id: event.pointerId } }
  const onPointerMove = (event: React.PointerEvent) => { const drag = dragRef.current; if (!drag) return; const distance = event.clientX - drag.x; const verticalDistance = event.clientY - drag.y; if (!drag.moved && Math.abs(distance) > 6 && Math.abs(distance) > Math.abs(verticalDistance)) drag.moved = true; if (drag.moved) { if (!rootRef.current?.hasPointerCapture(drag.id)) rootRef.current?.setPointerCapture(drag.id); event.preventDefault(); positionRef.current = drag.start - distance / Math.max(cardWidth * 0.35, 40); layout(positionRef.current) } }
  const onPointerUp = () => { const drag = dragRef.current; dragRef.current = null; if (drag?.moved) { suppressClickRef.current = true; goTo(Math.round(positionRef.current)) } }
  const onTouchStart = (event: React.TouchEvent) => { if (typeof window !== "undefined" && "PointerEvent" in window) return; if (data.length > 1) touchRef.current = { x: event.touches[0].clientX, y: event.touches[0].clientY, start: positionRef.current, moved: false } }
  const onTouchMove = (event: React.TouchEvent) => { if (typeof window !== "undefined" && "PointerEvent" in window) return; const touch = touchRef.current; const point = event.touches[0]; if (!touch || !point) return; const distance = point.clientX - touch.x; const verticalDistance = point.clientY - touch.y; if (!touch.moved && Math.abs(distance) > 6 && Math.abs(distance) > Math.abs(verticalDistance)) touch.moved = true; if (touch.moved) { event.preventDefault(); positionRef.current = touch.start - distance / Math.max(cardWidth * 0.35, 40); layout(positionRef.current) } }
  const onTouchEnd = () => { if (typeof window !== "undefined" && "PointerEvent" in window) return; const touch = touchRef.current; touchRef.current = null; if (touch?.moved) { suppressClickRef.current = true; goTo(Math.round(positionRef.current)) } }
  ...
  return <div ref={rootRef} className="depth-carousel" style={{ "--dc-perspective": `${perspective}px` } as React.CSSProperties} role="region" aria-roledescription="carousel" aria-label="Sản phẩm" tabIndex={0} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} onTouchCancel={onTouchEnd} onKeyDown={(event) => { if (event.key === "ArrowLeft") { event.preventDefault(); goTo(focusRef.current - 1) } if (event.key === "ArrowRight") { event.preventDefault(); goTo(focusRef.current + 1) } if (event.key === "Home") { event.preventDefault(); goTo(0) } if (event.key === "End") { event.preventDefault(); goTo(data.length - 1) } }}>
    <div className="depth-carousel__stage">{data.map((item, index) => <div key={index} ref={(element) => { cardRefs.current[index] = element }} className="depth-carousel__card" style={{ width: cardWidth, height: cardHeight }} role="group" aria-label={`${item.alt ?? "Sản phẩm"}, ${index + 1} / ${data.length}`} aria-current={active === index ? "true" : undefined} onClick={(event) => { if (suppressClickRef.current) { suppressClickRef.current = false; event.preventDefault(); event.stopPropagation(); return } goTo(index) }}>{item.content}</div>)}</div>
