...
export function ProductCatalog() {
  ...
  return <section id="bo-suu-tap" className="relative z-10 bg-hubi-cream pb-16 pt-8 text-hubi-deep-teal shadow-[0_-18px_40px_rgb(var(--hubi-ink-rgb)/.12)] sm:pb-20 sm:pt-6 lg:pb-24 lg:pt-8"><div className="mx-auto max-w-[1600px] px-5 sm:px-8 2xl:px-12"><ScrollReveal><h2 className="max-w-2xl font-serif text-3xl font-medium leading-[1.04] tracking-[-.035em] sm:text-4xl lg:text-5xl">Chọn điều đưa bạn đến gần mặt nước hơn.</h2></ScrollReveal><ScrollReveal delay={0.1} distance={16}><div className="mt-8 grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-5 lg:grid-cols-4" aria-label="Danh mục sản phẩm">{categoryCards.map((card) => <CategoryCard key={card.label} card={card} active={active === card.label} onSelect={() => setActive(card.label)} />)}</div></ScrollReveal><div className="mt-12 lg:mt-16">{categories.map((category) => <div key={category.label} hidden={category.label !== active} aria-hidden={category.label !== active}><CategorySection category={category} /></div>)}</div></div></section>
}
